...
class RemedialMeasuresCalculator {
  constructor(astrologer, geocodingService) {
    ...
    this.remedies = {
      ...
      mantras: {
        ...
        moon: ['Om Chandraya Namaha', 'Om Somaya Namaha', 'Shum Shukraya Namaha'],
        mars: ['Om Mangalaya Namaha', 'Om Angarakaya Namaha', 'Om Bhaumaya Namaha'],
        mercury: ['Om Buddhaya Namaha', 'Om Dhanvantre Namaha', 'Om Vignaya Namaha'],
        jupiter: ['Om Gurve Namaha', 'Om Brahmane Namaha', 'Om Vachaspataye Namaha'],
        venus: ['Om Shukraya Namaha', 'Om Lakshmyai Namaha', 'Om Kamadevaya Namaha'],
        saturn: ['Om Shanaischaraya Namaha', 'Om Sanaiscaraya Namaha', 'Om Nilambaraya Namaha'],
        rahu: ['Om Rahuve Namaha', 'Om Sarpaya Namaha', 'Om Chhayamartandaya Namaha'],
      ...
      charities: {
        ...
        mercury: ['Green items donation', 'Books/education support', 'Pen/paper donation'],
        jupiter: ['Yellow items donation', 'Teacher/guru dakshina', 'Temple donations'],
        venus: ['White items donation', 'Clothing donation', 'Milk products distribution'],
        saturn: ['Black items donation', 'Iron implements donation', 'Oil distribution'],
        rahu: ['Lead items donation', 'Root vegetables donation', 'Animal shelter support'],
        ketu: ['Dog food/shelter support', 'Grey items donation', 'Meditation cushions']
      },
      yantras: {
  ...
  async generateRemedialMeasures(birthData, analysis = null) {
    try {
      ...
      const chartAnalysis = analysis || await this._analyzeChartForRemedies(birthData);
      ...
      const specificRemedies = this._generateSpecificRemedies(afflictions, chartAnalysis);

      // Generate general preventive remedies
      const preventiveRemedies = this._generatePreventiveRemedies(chartAnalysis);

      // Generate timing recommendations for remedies
      const timingGuidelines = this._generateRemedyTiming(chartAnalysis, afflictions);
      ...
      return {
        ...
        timingGuidelines,
        spiritualPractices,
        summary: this._generateRemedialSummary(remedialPriority, specificRemedies, preventiveRemedies)
      };
    } catch (error) {
  ...
  async getPersonalizedRemedies(birthData, planetaryIssues) {
    try {
      ...
      const natalChart = await this._calculateBasicNatalChart(year, month, day, hour, minute, latitude, longitude, timezone);
  ...
  _identifyAfflictions(analysis) {
    ...
    Object.entries(analysis.problems?.planetary || {}).forEach(([planet, issues]) => {
    ...
    afflictions.house = this._identifyHouseAfflictions(analysis.problems?.houses || []);
  ...
  _generateSpecificRemedies(afflictions, chartAnalysis) {
    ...
    Object.entries(afflictions.planetary).forEach(([planet, affliction]) => {
      if (affliction.severity >= 3) { // High priority afflictions
        ...
        remedies.mantras.push({
          planet,
          mantras: this.remedies.mantras[planet] || [],
          recitation: this._getRecitationGuidelines(planet, affliction.severity)
        });
  ...
  _generateRemedyTiming(chartAnalysis, afflictions) {
    ...
    // Periods to avoid
    timing.avoidancePeriods = this._identifyAvoidancePeriods(chartAnalysis, afflictions);
  ...
  async _calculateBasicNatalChart(year, month, day, hour, minute, latitude, longitude, timezone) {
    const natalPlanets = {};
    ...
    natalPlanets.ascendant = { longitude: houses.ascendant, sign: Math.floor(houses.ascendant / 30) + 1 };
  ...
  _selectPrimarilyRemedies(planet, issues) {
    ...
    if (issues.includes('retrograde')) {
      remedies.push('special_mantras', 'yantra_installation', 'spiritual_practices');
    }
  ...
  _generatePriorityRecommendations(priority) {
    ...
    if (priority.urgency === 'high') {
      recommendations.push('Immediate consultation with spiritual guide recommended');
      recommendations.push('Start basic mantras and charitable activities right away');
      recommendations.push('Consider professional astrological guidance for proper implementation');
      ...
      recommendations.push('Consult experienced practitioner for personalized guidance');
    } else {
  ...
  _generatePlanetaryPractices(planet, type) {
    const practices = {
      general: ['Daily spiritual practice', 'Positive thinking', 'Service activities'],
      malefic: ['Fasting rituals', 'Charitable works', 'Devotional hymns'],
      benefic_but_afflicted: ['Strengthening prayers', 'Good deeds', 'Spiritual study']
    };
  ...
  _assessPlanetaryProblems(natalChart) {
    ...
    Object.entries(natalChart.planets).forEach(([planet, data]) => {
      if (planet !== 'ascendant') {
        ...
        if (planet !== 'sun' && Math.abs(data.longitude - natalChart.planets.sun.longitude) < 8) {
  ...
  _assessHouseProblems(natalChart) {
    ...
    const seventhPlanets = Object.values(natalChart.planets).filter(p => p.house === 7);
