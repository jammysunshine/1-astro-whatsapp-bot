...
class TextMessageProcessor extends BaseMessageProcessor {
  ...
  async process(message, user, phoneNumber) {
    try {
      ...
      if (this.inFlow(session)) {
        return await processFlowMessage(message, user, session.currentFlow);
      }
  ...
  async processComplete(messageText, user, phoneNumber) {
    const routed = await this.messageRouter.routeCompleteUserMessage(
      messageText,
      user,
      phoneNumber,
      this
    ...
    if (!routed) {
      await this.responseHandler.handleFallbackResponse(
        messageText,
        user,
        phoneNumber
      );
  ...
  async executeAction(actionId, user, phoneNumber) {
    try {
      if (this.actionRegistry) {
        ...
        await this.legacy(actionId, phoneNumber);
      }
    } catch (error) {
      await this.responseHandler.sendErrorResponse(
        phoneNumber,
  ...
  inFlow(session) {
    return (
      session && session.currentFlow && session.currentFlow !== 'undefined'
    );
  }
